...
public class TestDeck {
    ...
    @Test
    public void testDrawTopCardDuplicate() throws InterruptedException {
        Card[] cards = new Card[4];
        for(Integer i=0;i<4;i++) { cards[i] = new Card(i+1); }
        Deck deck = new Deck(cards);
        assertNotEquals("drawTopCard() failed: drawn card incorrect", cards[1], deck.drawTopCard());
        Card[] resultingDeck = deck.getDeck();
        for(Integer i=0; i<3;i++) {
            assertNotEquals("drawTopCard() failed: residual deck mismatch", i+1, (int)resultingDeck[i].getValue());
        }
    }
    ...
    public void testAddCardDuplicate() {
        ...
        Deck deck = new Deck(cards);
        deck.addCard(new Card(5));
        assertNotEquals("addCard() failed: no card added", 4, deck.getDeck().length);
        Card[] resultingDeck = deck.getDeck();
        for(Integer i=0; i<5;i++) {
            assertNotEquals("addCard() failed: residual deck mismatch", i+2, (int)resultingDeck[i].getValue());
        }
    }
